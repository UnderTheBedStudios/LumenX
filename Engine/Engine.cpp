...
std::unique_ptr<Model> g_CameraModel;
...
void Engine_Shutdown()
{
    ...
    g_MainShader.reset();
    g_DepthShader.reset();
    g_CameraModel.reset(); // Model's dtor -> ~Mesh() per mesh, cleans up VAO/VBO/EBO/textures
}
...
void Engine_Init(void* getProcAddress, const char* assetRoot)
{
    ...
    g_CameraModel = std::make_unique<Model>();
    std::string cameraModelPath = g_AssetRoot + "/Engine/Models/Camera/Camera.obj";
    if (!g_CameraModel->LoadFromFile(cameraModelPath))
...
void Engine_RenderFrame(int fb, int width, int height, const float* viewProj, const float* model)
{
    ...
    if (g_CameraModel)
        g_CameraModel->Draw(); // each Mesh binds its own diffuse texture to unit 0 before drawing
}
